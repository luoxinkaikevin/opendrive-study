...
namespace PlannerHNS
{
        ...
        const std::vector<std::pair<STPoint, STPoint>> &point_pairs)
    {
        ...
        for (const auto &item : reduced_pairs)
        {
            ...
            const double t = item.first.x;
            lower_points_.emplace_back(item.first.y, t);
            upper_points_.emplace_back(item.second.y, t);
        }

        for (auto it = lower_points_.begin(); it != lower_points_.end(); ++it)
        {
            points_.emplace_back(it->x, it->y, 0, 0);
        }
        for (auto rit = upper_points_.rbegin(); rit != upper_points_.rend(); ++rit)
        {
            points_.emplace_back(rit->x, rit->y, 0, 0);
        }
        ...
        std::vector<std::pair<STPoint, STPoint>> *point_pairs)
    {
        ...
        {
            GPSPoint line_f(
                point_pairs->at(i).first.x, point_pairs->at(i).first.y, 0, 0);
            GPSPoint line_ff(
                point_pairs->at(j + 1).first.x, point_pairs->at(j + 1).first.y, 0, 0);
            GPSPoint line_s(
                point_pairs->at(i).second.x, point_pairs->at(i).second.y, 0, 0);
            GPSPoint line_ss(
                point_pairs->at(j + 1).second.x, point_pairs->at(j + 1).second.y, 0, 0);
            GPSPoint line_jf(
                point_pairs->at(j).first.x, point_pairs->at(j).first.y, 0, 0);
            GPSPoint line_js(
                point_pairs->at(j).second.x, point_pairs->at(j).second.y, 0, 0);
            math::LineSegment2d lower_seg(line_f, line_ff);
            math::LineSegment2d upper_seg(line_s, line_ss);
            if (!IsPointNear(lower_seg, line_jf, kMaxDist) ||
                !IsPointNear(upper_seg, line_js, kMaxDist))
            {
                ++i;
                if (i != j)
                {
    ...
    bool StBoundary::IsPointInBoundary(const STPoint &st_point) const
    {
        ...
        const double check_upper =
            (upper_points_[left] - st_point).CrossProd(upper_points_[right] - st_point);

        const double check_lower =
            (lower_points_[left] - st_point).CrossProd(lower_points_[right] - st_point);
