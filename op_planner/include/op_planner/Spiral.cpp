...
*/
                     ...
                     double gamma, double s, double &real, double &imag, double &theta)
    {
        if (gamma == 0)
        {
            printf("error: eulerSpiral curvture is zero");
        }
        double Sa, Ca;
        double Sb, Cb;
        fresnel((kappa0 + gamma * s) / sqrt(M_PI * fabs(gamma)), &Sa, &Ca);
        fresnel(kappa0 / sqrt(M_PI * fabs(gamma)), &Sb, &Cb);

        double a = sign(gamma);
        double b = Ca - Cb;
        double c = Sa - Sb;
        double d = sqrt(M_PI / fabs(gamma));
        double f = theta0 - kappa0 * kappa0 / gamma / 2.0;

        real = x0 + a * b * d * cos(f) - d * c * sin(f);
        imag = y0 + a * b * d * sin(f) + d * c * cos(f);

        theta = theta0 + kappa0 * s + gamma * s * s / 2.0;
    }
